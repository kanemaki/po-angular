...
export class PoPageDynamicService {
  ...
  deleteResource(id, endpoint = this.endpoint): Observable<any> {
    return this.http.delete(`${endpoint}/${id}`, { headers: this.headers });
  }

  // Deleta recursos em lote
  deleteResources(ids: Array<any>, endpoint = this.endpoint): Observable<any> {
    return this.http.request('delete', `${endpoint}`, { headers: this.headers, body: ids });
  }

  // Busca uma lista de recursos
  getResources(params?: HttpParams, endpoint = this.endpoint): Observable<any> {
    return this.http.get(endpoint, { headers: this.headers, params });
  }

  // Busca um único recurso
  getResource(id, endpoint = this.endpoint): Observable<any> {
    return this.http.get(`${endpoint}/${id}`, { headers: this.headers });
  }

  // Cria um recurso
  createResource(resource, endpoint = this.endpoint): Observable<any> {
    return this.http.post(`${endpoint}`, resource, { headers: this.headers });
  }

  // Atualiza um recurso
  updateResource(id, resource, endpoint = this.endpoint): Observable<any> {
    return this.http.put(`${endpoint}/${id}`, resource, { headers: this.headers });
  }
}
